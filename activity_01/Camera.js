class Camara {
    constructor(cameraPos, cameraHeadVec, cameraLookVec) {
        this.directionControl = [0, 0, 0];
        this.cameraControl = [0, 0, 0];

        this.cameraPos = cameraPos;

        this.cameraHeadVec = new Vector3(
                [
                    cameraHeadVec.elements[0],
                    cameraHeadVec.elements[1],
                    cameraHeadVec.elements[2]
                ])
            .normalize();
        this.cameraLookVec = new Vector3(
            ...
            .normalize();
        this.cameraRightVec =
            this.cross(this.cameraLookVec, this.cameraHeadVec)
            .normalize();

        this.cHead = new Vector3(
                [
                    cameraHeadVec.elements[0],
                    cameraHeadVec.elements[1],
                    cameraHeadVec.elements[2]
        ...
        lookAtMatrix.setLookAt(
            ...
            cameraHeadVec.elements[0], cameraHeadVec.elements[1], cameraHeadVec.elements[2]
        );

        // let tempQuat = this.matToQuat(lookAtMatrix);
        // let tempMat = new Matrix4();
        // tempMat.setFromQuat(tempQuat.x, tempQuat.y, tempQuat.z, tempQuat.w);
        // console.log(tempMat, lookAtMatrix);

        this.qNew = new Quaternion(0, 0, 0, 1);
        this.qBase = this.matToQuat(lookAtMatrix);
        this.qTot = new Quaternion(0, 0, 0, 1);
        // this.qTot.setFromAxisAngle(1.0, 0.0, 0.0, 95);
    ...
    matToQuat(matrix) {
        ...
        /**
          // finded on http://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToQuaternion/
    ...
    calculateQuaternions(elapsedTime) {
        ...
        if (this.directionControl[2] != 0) {
            ...
            this.qNew.setFromAxisAngle(0.0, 1.0, 0.0, this.rotateSpeed * elapsedTime * this.directionControl[2]);
        ...
        if (this.directionControl[0] != 0) {
            qTemp = new Quaternion(0, 0, 0, 1);
            this.qNew.setFromAxisAngle(0.0, 0.0, 1.0, this.rotateSpeed * elapsedTime * this.directionControl[0]);

            qTemp.multiply(this.qNew, this.qTot);
            this.qTot.copy(qTemp);
        }

        this.multiplyVec3(this.qTot, this.cameraHeadVec, this.cHead);
        this.multiplyVec3(this.qTot, this.cameraLookVec, this.cLook);
        this.multiplyVec3(this.qTot, this.cameraRightVec, this.cRight);
        ...
        qTemp = new Quaternion(0, 0, 0, 1);
        // qTemp.setFromAxisAngle(0, 0, 0, 0);
