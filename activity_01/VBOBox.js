class VBOBox {
    constructor(gl, vShader, fShader) {
        ...
        this.yFov = 35;
        this.camera = new Camara(new Vector3([0, 12, 4]), new Vector3([0, -4, 12]), new Vector3([0, -12, -4]));
    ...
    initParticle() {
        ...
        this.kDrag = 0.985;
        this.fGrav = 9.832;
        this.restituition = 1.0;

        this.posPrev = [];
        // Change this thing to particle count * 3
        // Don't use model matrix!
        this.posNow = [];
        this.velPrev = [];
        this.velNow = [];
        ...
        this.stateNow = new ParticleState();
        this.statePrev = new ParticleState();

        for (let i = 0; i < this.ballCount; i++) {
            let particleNow = new Particle();
            particleNow.pos = [Math.random() * 4.0 - 2.0, Math.random() * 4.0 - 2.0, Math.random() - 0.5];
            particleNow.vel = [0.0, 0.0, 0.0];
            this.stateNow.particles.push(particleNow);
            let particlePrev = new Particle();
            this.statePrev.particles.push(particlePrev);
        }
    ...
    calcParticles(elapsedTime) {
        ...
        if (mode > 1) {
            ...
            for (let i = 0; i < this.ballCount; i++) {
                this.calcParticle(elapsedTime, i);
            }
        }

        // let startPoint = this.startPoints.particle.verts * this.floatsPerVertex;
        // this.particleSystem.updateStatePreRender();
        // for (let i = 0; i < this.particleSystem.particleCount; i++) {
        //     let particle = this.particleSystem.currState[i];
        //     for (let j = 0; j < 3; j++) {
        //         this.vertices[startPoint + i * 9 + j] = particle.pos[j];
        //     }
        // }
        // this.particleSystem.updateStatePostRender(elapsedTime);
    }

    calcParticle(elapsedTime, index) {

        let particleNow = this.stateNow.particles[index];
        let particlePrev = this.statePrev.particles[index];

        for (let i = 0; i < 3; i++) {
            particlePrev.pos[i] = particleNow.pos[i];
            particlePrev.vel[i] = particleNow.vel[i];
        }

        particleNow.vel[2] -= this.fGrav * elapsedTime;

        particleNow.vel[0] *= this.kDrag;
        particleNow.vel[1] *= this.kDrag;
        particleNow.vel[2] *= this.kDrag;

        particleNow.pos[0] += particleNow.vel[0] * elapsedTime;
        particleNow.pos[1] += particleNow.vel[1] * elapsedTime;
        particleNow.pos[2] += particleNow.vel[2] * elapsedTime;

        var startPoint = this.startPoints.particle.verts * this.floatsPerVertex;

        for (let i = 0; i < 3; i++) {

            if (particleNow.pos[i] < -2.0 && particleNow.vel[i] < 0.0) {
                particleNow.pos[i] = -2.0;
                particleNow.vel[i] = particlePrev.vel[i];
                particleNow.vel[i] *= this.kDrag;

                particleNow.vel[i] = particleNow.vel[i] < 0.0 ?
                    -this.restituition * particleNow.vel[i] : this.restituition * particleNow.vel[i];
            } else if (particleNow.pos[i] > 2.0 && particleNow.vel[i] > 0.0) {
                particleNow.pos[i] = 2.0;
                particleNow.vel[i] = particlePrev.vel[i];
                particleNow.vel[i] *= this.kDrag;
            ...
            this.vertices[startPoint + index * 9 + i] = particleNow.pos[i];
        }
    }
    ...
    flingBall() {
        for (let j = 0; j < this.ballCount; j++) {
            for (let i = 0; i < 3; i++) {
                ...
                this.stateNow.particles[j].vel[i] +=
                    this.stateNow.particles[j].vel[i] > 0.0 ? v : -v;
            }
        }
