...
export default class VBOBox {
    constructor(gl) {
        ...
        this.camera = new Camera(new Vector3([0, 0, 0]), new Vector3([1, 0, 0]), new Vector3([0, 0, -1]));
    ...
    initTexture() {
        ...
        for (let j = 0; j < this.imgYmax; j++) { // for the j-th row of pixels
            for (let i = 0; i < this.imgXmax; i++) { // and the i-th pixel on that row,
                ...
                if ((i % 26) * (j % 26) == 0) {
                    ...
                    this.myImg[idx + 1] = 255; // 0 <= grn <= 255
