...
var vShaderGauroud =
    ...
    '\n' +
    '  vec4 ambient = vec4(GauroudLight[lightIndex].La, 1.0) * VertexColor * GauroudMaterial.Ka;\n' +
    '  vec4 diffuse = vec4(lambertian * GauroudLight[lightIndex].Ld, 1.0) * VertexColor * GauroudMaterial.Kd;\n' +
    '\n' +
    ...
    '  if ( lambertian > 0.0 )\n' +
    '    spec = vec4(GauroudLight[lightIndex].Ls, 1.0) * VertexColor * GauroudMaterial.Ks * pow( angle, GauroudMaterial.Shininess );\n' +
    '\n' +
...
var fShaderParticle =
    ...
    '  if(dist < 0.5) { \n' +
    '  	gl_FragColor = vec4((0.5 - dist + 0.5)*FrontColor.rgb, FrontColor.a);\n' +
    '  } else { discard; }\n' +
    '}\n' +
    '\n';
...
function initVBOs() {
    ...
    vboParticle.addModelBuilder("particles", (vbo, elapsedTime) => {
        let startPoint = vbo.startPoints["particles"].verts * ShapeBuilder.BYTES_PER_ELEMENT;
        for (let i = 0; i < particleSystem.particleCount; i++) {
            let particle = particleSystem.state[i];
            for (let j = 0; j < 3; j++) {
                vbo.vertices[startPoint + i * ShapeBuilder.BYTES_PER_ELEMENT + j] = particle.pos[j];
            }
            for (let j = 0; j < 4; j++) {
                vbo.vertices[startPoint + i * ShapeBuilder.BYTES_PER_ELEMENT + j + 3] = particle.color[j];
            }
            vbo.vertices[startPoint + i * ShapeBuilder.BYTES_PER_ELEMENT + 10] = particle.size;
        }
    ...
    vboParticle.build(vShaderGauroud, fShaderParticle);
}
